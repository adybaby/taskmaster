/* This is a copy of redux-query-sync on github with only one line [131] changed:
 * replaced: "if (value === defaultValue) {"
 * with: "if (JSON.stringify(value) === JSON.stringify(defaultValue)) {"
 * This is just to ensure that array comparisons work when comparing default values
 * */
 ...
 */
function ReduxQuerySync({ store, params, replaceState, initialTruth, history = createHistory() }) {
  ...
  function getLocation(history) {
    ...
    if (Object.hasOwnProperty.call(history, 'getCurrentLocation')) {
